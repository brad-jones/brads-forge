...
## Example 8: Platform-Conditional Runtime Dependency (mkcert)

When a runtime dependency is only available/needed on some platforms, declare `requirements` as a function instead of a
static object. It receives a `RequirementsContext` and is resolved once per `targetPlatform` during generation.
...
- `requirements` may be `(ctx: RequirementsContext) => Requirements | Promise<Requirements>` instead of a static object
  — useful when a dependency isn't available/needed on every supported platform.
- Recipe generation is always per-platform (`task generate` calls `Recipe.toObject(targetPlatform)` once per target), so
  the function resolves unconditionally — no `if`/`then` selectors are needed in the generated YAML.
- `ctx.unix` is `true` for `linux-*`/`osx-*` and `false` for `win-*`.
